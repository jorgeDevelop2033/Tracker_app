...
{
    public sealed class PorticoRepository(TrackerDbContext db) : EfRepositoryBase<Portico>(db), IPorticoRepository
    {
        ...
        public async Task<IReadOnlyList<Portico>> GetNearAsync(Point position4326, double maxDistanceMeters, int take = 20, CancellationToken ct = default)
        {
            // Para SQL Server geography, usar STDistance. EF Core con NTS traduce Distance.
        ...
        public async Task<IReadOnlyList<Portico>> IntersectsCorredorAsync(LineString corredor4326, int take = 50, CancellationToken ct = default)
        {
            return await _db.Porticos
            .AsNoTracking()
